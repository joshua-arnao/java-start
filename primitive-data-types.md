...
#### RESERVA DE MEMORIA POR REGIONES (G1) — evidencia con PoC

Al crear un array grande, la JVM (con el recolector G1, default en Java 21)
NO reserva exactamente los bytes que necesitas. Reserva el heap en
**regiones de tamaño fijo** (el tamaño se calcula automáticamente según
el heap máximo configurado — en el PoC de prueba salió 4MB por región,
pero varía según cada proceso). Cuando tu array necesita espacio, la
JVM le asigna el número MÍNIMO de regiones completas que alcanzan a
cubrirlo — nunca "media región".

> Evidencia real (PoC con 10,000,000 elementos):
> - `int[]`  → necesitaba 38.15 MB reales → JVM reservó 40 MB   (10 regiones de 4MB)
> - `byte[]` → necesitaba 9.54 MB reales  → JVM reservó 12 MB   (3 regiones de 4MB)
>
> Por eso medir memoria con `Runtime.getRuntime()` es poco confiable
